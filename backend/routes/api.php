...
// ──────────────────────────────────────────────────────────────────────────────
// DANGER ZONE: Admin-only maintenance endpoints
// These MUST be behind authentication + admin-role check.
// ──────────────────────────────────────────────────────────────────────────────
Route::middleware('auth:sanctum')->group(function () {
    Route::get('/trigger-migrate', function (\Illuminate\Http\Request $request) {
    ...
    Route::get('/seed-demo', function (\Illuminate\Http\Request $request) {
        if ($request->user()->role !== 'ADMIN') {
            return response()->json(['message' => 'Forbidden'], 403);
        }
        if (!in_array(config('app.env'), ['local', 'staging'])) {
...
Route::get('/storage/plans/{filename}', function ($filename) {
    ...
    $path = storage_path('app/public/plans/' . $safeFilename);
...
Route::post('/preview-file', function (\Illuminate\Http\Request $request) {
    try {
        ...
        // ── SECURITY: Prevent Local File Inclusion (Path Traversal) ──
        // 1. Resolve the intended base directory
        $allowedDir = realpath(storage_path('app/public'));
        // 2. Build and resolve the full requested path
        $fullPath = realpath(storage_path('app/public/' . $filePath));

        // 3. Reject if realpath failed (file doesn't exist) or the resolved
        //    path escapes outside the allowed directory.
